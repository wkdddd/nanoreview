"""Utility functions for nanoreview."""
...
_TOOL_RESULTS_DIR = ".nanoreview/tool-results"
...
def split_message(content: str, max_len: int = 2000) -> list[str]:
    ...
    Args:
        ...
        max_len: Maximum length per chunk (default 2000 for client compatibility).
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 nanoreview v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("nanoreview") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
