...
from nanoreview.config.paths import get_media_dir, get_runtime_subdir
from nanoreview.config.schema import Base
from nanoreview.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.nanoreview/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'nanoreview channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"nanoreview-{uuid.uuid4().hex[:12]}"
