...
# Remove default handler and re-add with unified nanoreview format
logger.remove()
...
from nanoreview.cli.stream import StreamRenderer, ThinkingSpinner
from nanoreview.config.paths import get_workspace_path
from nanoreview.config.schema import Config
from nanoreview.utils.helpers import sync_workspace_templates
from nanoreview.utils.restart import (
...
app = typer.Typer(
    ...
    help=f"{__logo__} nanoreview - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from nanoreview.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} nanoreview[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} nanoreview[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} nanoreview v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """nanoreview - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize nanoreview configuration and workspace."""
    from nanoreview.config.loader import get_config_path, load_config, save_config, set_config_path
    from nanoreview.config.schema import Config
    ...
    if wizard:
        from nanoreview.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'nanoreview onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'nanoreview agent -m "Hello!"'
    gateway_cmd = "nanoreview gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} nanoreview is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from nanoreview.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from nanoreview.config.loader import get_config_path
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show nanoreview runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
...
def gateway(
    ...
    from nanoreview.utils.gateway_logging import configure_gateway_file_logging
...
def _run_gateway(
    ...
    *,
    ...
    from nanoreview.channels.websocket import publish_runtime_model_update
    from nanoreview.providers.factory import build_provider_snapshot, load_provider_snapshot
    from nanoreview.session.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting nanoreview gateway version {__version__} on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show nanoreview runtime logs during chat"),
):
...
def _check_fail_on(report_content: str, threshold: str) -> int:
    ...
    from nanoreview.review import SEVERITY_ORDER
...
def channels_status(
    ...
    """Show channel status."""
    from nanoreview.channels.registry import discover_all
    from nanoreview.config.loader import load_config, set_config_path
...
def plugins_list():
    ...
    from nanoreview.channels.registry import discover_all, discover_channel_names
...
def status():
    ...
    console.print(f"{__logo__} nanoreview Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from nanoreview.providers.registry import PROVIDERS
