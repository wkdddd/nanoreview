...
`nanoreview/config/loader.py` resolves `${VAR}` references through `resolve_config_env_vars`. This is not shell default-value syntax. A missing variable raises `ValueError`; retain that explicit failure instead of silently substituting a value.

## Prompt and Context Surfaces

Templates in `nanoreview/templates/`, tool descriptions, skills and replayed session history affect LLM behavior as directly as Python code. Keep changes narrow and test them where possible. Do not teach the model to reproduce internal markers, raw paths or tool-call transcripts.
